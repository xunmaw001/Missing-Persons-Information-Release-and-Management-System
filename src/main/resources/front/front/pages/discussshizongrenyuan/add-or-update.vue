	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussshizongrenyuan`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussshizongrenyuan`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussshizongrenyuan`, this.ruleForm);
						}else{
							await this.$api.add(`discussshizongrenyuan`, this.ruleForm);
						}
						uni.setStorageSync('discussshizongrenyuanCleanType',true);
